  ...
  async function onSubmit(values: z.infer<typeof EventFormSchema>) {
    ...
    if (type === "Update") {
      try {
        ...
        const updatedEvent = await updateEvent({ userId, event: { ...values, imageUrl: uploadedImageUrl, _id: eventId }, path: `/events/${eventId}` });
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="w-full">
                <FormControl>
                  <div className="flex-center h-[54px] w-full overflow-hidden rounded-full bg-gray-50 px-4 py-2">
                    <Image src="/assets/icons/dollar.svg" alt="doller" width={24} height={24} className="filter-grey" />
